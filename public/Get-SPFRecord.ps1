...
function Get-SPFRecord {
    ...
    param(
        [Parameter(
            ...
            HelpMessage = "SPF kayitlari cozumlenecek bir veya birden fazla domain adi girin.")]
        [string[]]$Name,
    ...
    Process {
        foreach ($domain in $Name) {
            ...
            # Belirtilen domain'in SPF kaydini getir
            ...
            # SPF 'redirect' yonlendirmesi var mi kontrol et ve varsa takip et
            ...
            # Birden fazla SPF kaydi var mi kontrol et
            # (RFC 4408: domain basina yalnizca BIR SPF kaydi olabilir)
            ...
            # SPF DNS lookup sayacini baslat
            # SPF kaydi 10 DNS lookup sinirini ASMAMALIDIR
            # See: https://datatracker.ietf.org/doc/html/rfc7208#section-4.6.4
            Write-Verbose "Starting calculation of SPF DNS Lookup count"
            $SpfDnsLookupCount = 0
            $includedDomain

            # DNS lookup sinirina dahil olan mekanizmalari al
            # (include:, a, a:, mx, mx:, ptr sayilir; ip4: ve ip6: sayilmaz)
